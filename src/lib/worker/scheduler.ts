...
export async function initializeWorkerEngine() {
  ...
  try {
    ...
    console.warn('⚠️ Redis not detected locally. Falling back to built-in In-Memory Worker Scheduler.');
...
async function syncBullMQSchedules() {
  ...
  const monitorsRes = await query('SELECT * FROM monitors WHERE is_active = true');
...
function startInMemoryScheduler() {
  ...
  console.log('⏳ In-Memory Scheduler started. Checking monitors periodically...');

  setInterval(async () => {
    try {
      const activeMonitorsRes = await query('SELECT * FROM monitors WHERE is_active = true');
      const now = Date.now();
      ...
      for (const monitor of activeMonitorsRes.rows) {
        ...
        const lastTestedAt = lastLogRes.rows[0]?.tested_at ? new Date(lastLogRes.rows[0].tested_at).getTime() : 0;
        const intervalMs = (monitor.interval_sec || 60) * 1000;

        if (now - lastTestedAt >= intervalMs) {
          console.log(`[WORKER] Ping interval reached for "${monitor.name}" (${monitor.url})`);
          executePingCheck(monitor.id).catch((err) => {
            console.error(`[WORKER ERROR] Ping failed for ${monitor.name}:`, err.message);
          });
        }
      }
    } catch (err: unknown) {
